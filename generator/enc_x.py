...
class MPNEncoder(nn.Module):
    ...
    def forward(self, fnode, fmess, agraph, bgraph):
        h = self.rnn(fmess, bgraph)  # lstm
        h = self.rnn.get_hidden_state(h)
        nei_message = index_select_ND(h, 0, agraph)  # 以agraph为索引，将h转换---->即只接受相邻节点的消息
...
class HierMPNEncoder(nn.Module):
    ...
    def __init__(self, vocab, vocab_atom, rnn_type, embed_size, hidden_size, depthT, depthG, dropout):
        super(HierMPNEncoder, self).__init__()
        self.vocab = vocab
        self.hidden_size = hidden_size
        self.dropout = dropout
        self.atom_size = atom_size = vocab_atom.size()
...
class IncHierMPNEncoder(HierMPNEncoder):
    ...
    def __init__(self, vocab, vocab_atom, rnn_type, embed_size, hidden_size, depthT, depthG, dropout):
        super(IncHierMPNEncoder, self).__init__(vocab, vocab_atom, rnn_type, embed_size, hidden_size, depthT, depthG,
                                                dropout)
