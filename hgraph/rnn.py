...
class LSTM(nn.Module):
    ...
    def lstm(self, x, h_nei, c_nei):
        ...
        o = self.W_o(torch.cat([x, h_sum_nei], dim=-1))  # o = w[h,x]+b
        f = self.W_f(torch.cat([x_expand, h_nei], dim=-1))  # f = w(h,x)+b
        u = self.W(torch.cat([x, h_sum_nei], dim=-1))  # c~ = u = tanh(w[h,x]+b)
        c = i * u + (f * c_nei).sum(dim=1)  # c = f*c + i*u
        h = o * torch.tanh(c)  # h =  o *tanh(c)
        return h, c

    def forward(self, fmess, bgraph):
        h = torch.zeros(fmess.size(0), self.hidden_size, device=fmess.device)  # lstm的输出门
        c = torch.zeros(fmess.size(0), self.hidden_size, device=fmess.device)  # 遗忘门
        mask = torch.ones(h.size(0), 1, device=h.device)
        mask[0, 0] = 0  # first message is padding
